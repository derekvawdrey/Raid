...
public class TeleportAccept implements CommandExecutor {
    ...
    public TeleportAccept(JavaPlugin plugin, DataAccessManager dataAccessManager) {
        this.plugin = plugin;
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] strings) {
        ...
        if (!player.hasPermission("factionsrevived.tpaccept")) {
        ...
        if (strings.length != 1) {
            MessageManager.sendMessage(player, "Usage: /tpaccept <activatorName>");
            return true;
        }
        ...
        if (teleportRequest != null) {
            ...
            teleportRequest.handleTeleportationResponse(true, dataAccessManager);
        } else {
